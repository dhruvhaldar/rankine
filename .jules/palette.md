...
## 2026-08-16 - Accessible Loading Animations
**Learning:** When adding CSS animated loading spinners (e.g., replacing static emojis during form submission), some users with vestibular sensitivities can be adversely affected by continuous looping animations.
**Action:** Always include a `prefers-reduced-motion: reduce` fallback (via CSS media query or JS `window.matchMedia`) to revert to a static visual for users with reduced motion preferences.
