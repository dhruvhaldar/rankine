...
**Learning:** When adding CSS animated loading spinners (e.g., replacing static emojis during form submission), failure to provide an accessible fallback can cause issues for users with vestibular sensitivities.
**Action:** Always include a `prefers-reduced-motion: reduce` fallback (via CSS media query or JS `window.matchMedia`) to revert to a static visual for animated loading states.
