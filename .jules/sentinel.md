...
## 2026-08-10 - Fix Logging DoS via Malformed String Interpolation
**Vulnerability:** The `SecurityContextFilter` dynamically assigned formatted messages to `record.msg` and cleared `record.args` to avoid re-formatting. However, if the interpolated string contained `%` characters (e.g., `%s`), the downstream `StreamHandler` would still call `record.getMessage()`. Since `record.args` was empty (`()`), the native `msg % self.args` operation would crash the application with a `TypeError: not enough arguments for format string`.
**Learning:** In Python `logging.Filter`, manual interpolation like `record.msg % record.args` can raise TypeErrors, and assigning formatted strings directly to `record.msg` can crash the logger if the injected content contains `%` characters while `record.args` is present or subsequently evaluated as empty against a string containing format specifiers.
**Prevention:** Always safely resolve log messages inside filters by wrapping `record.getMessage()` in a `try...except` block, falling back to `str(record.msg)` on failure, and explicitly clearing `record.args` to `()` before appending dynamic content to avoid DoS via malformed log messages.
