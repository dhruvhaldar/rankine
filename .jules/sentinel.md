...
## 2024-08-30 - Fix Filter Injection Crash in Logging Filter
**Vulnerability:** Injecting dynamically formatted strings directly into `record.msg` within a `logging.Filter` can crash the Python logger if the injected content contains `%` characters and `record.args` is present. Furthermore, manual interpolation like `record.msg % record.args` can raise TypeErrors.
**Learning:** Manual message interpolation inside logging filters is fragile. If an attacker controls the payload and triggers a log, they can inject malformed formatting strings or mismatched arguments to cause `TypeError` or formatting errors, crashing the application (DoS).
**Prevention:** Safely resolve log messages inside filters by wrapping `record.getMessage()` in a `try...except`, falling back to `str(record.msg)` on failure, and explicitly clearing `record.args` to `()` before appending dynamic content to avoid DoS via malformed log messages.
