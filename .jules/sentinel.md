...
## 2026-08-21 - Fix Header Redaction Bypass via Null Bytes
**Vulnerability:** The `SecurityContextFilter` redacted sensitive HTTP headers by stripping a set of whitespace characters from the header key (e.g., `.strip(' \t\n\r\x0b\x0c:')`) before checking if it was in a sensitive list. Attackers could bypass this redaction by appending a null byte (`\x00`) to the header name (e.g., `Cookie\x00`), causing the check to fail and logging sensitive values (like session cookies) in plaintext.
**Learning:** High-level frameworks and web servers handle malformed headers differently, sometimes tolerating or normalizing null bytes. Failing to account for null bytes in custom header sanitization creates a bypass vector. High-level HTTP clients like `requests` will block null bytes in headers locally, requiring raw sockets or framework test clients to verify.
**Prevention:** Always include the null byte (`\x00`) in the character list when stripping whitespace/padding from user-controlled keys for security filters (e.g., `.strip(' \t\n\r\x0b\x0c\x00:')`).
