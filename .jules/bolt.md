...
## 2026-09-18 - Algebraic Square Root Consolidation for Stagnation Ratios
**Learning:** Calculating large mathematical products involving multiple square roots introduces significant array allocation overhead for aerodynamic property arrays. Patterns like `(A**3 / B**2) * sqrt(A/B)` or `(A**3 * B**2) * sqrt(A * B)` are inefficient and slow because of repeated identical operations, especially on NumPy arrays.
**Action:** When computing products or quotients involving multiple square roots (e.g., `(A^3 * B^2) * sqrt(A * B)` or `(A^3 / B^2) * sqrt(A / B)`), consolidate the operations algebraically into simpler products using an intermediate `X` term to minimize function calls. For instance, rewrite `P02_P_inf = (t1 * t1 * t1 * t2 * t2) * sqrt(t1 * t2)` as `X = t1 * t2; P02_P_inf = t1 * X * X * sqrt(X)`. This reduces overhead and speeds up calculations significantly.
